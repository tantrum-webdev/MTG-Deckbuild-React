...
describe('This is the mock for the local storage', () => {
  ...
  it('setItem: Add a new item to storage', () => {
  ...
  it('setItem: Adding a new item to existing key overwrites old value', () => {
  ...
  it('getItem: Retrieve one item from storage', () => {
    mockStorage.items.set('test', 'item');

    expect(mockStorage.getItem('test')).toBe('item');
  });

  it('getItem: Returns null if key does not exist', () => {
